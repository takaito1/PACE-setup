...
INTERMED_DIR = BASE_DIR / "intermed_files_NCEI"
OUT_FILE = BASE_DIR / "o2_NCEI_mon_clim_65C5.nc"
...
def main():
    ...
    first_file = INTERMED_DIR / f"o2map_mon1_klev1.mat"
    with h5py.File(first_file, "r") as f:
        x = np.asarray(f["x"][:]).ravel()
        y = np.asarray(f["y"][:]).ravel()
        z = np.asarray(f["z"][:]).ravel()

    an = np.full((len(month), NLEV, len(y), len(x)), np.nan, dtype=np.float64)

    for mi, mon in enumerate(month):
        for k in range(1, NLEV + 1):
            fn = INTERMED_DIR / f"o2map_mon{mon}_klev{k}.mat"
    ...
    T = (np.asarray(month, dtype=np.float64)/12+2025-1/24-1980)*365.25  # raw month, not converted to days-since (see docstring)
    ...
    with nc.Dataset(OUT_FILE, "w", format="NETCDF4") as ds:
        ...
        ds.title = "objectively mapped dissolved oxygen based on NCEI QC data"
        ...
        vo.long_name = "objective map of dissolved oxygen based on NCEI QC data"
