% %%% objmap_o2_annual_clim_multi
% %%% Single-level OI worker for the Oxygen_<source>_1x1bin_ann_clim_lon0to360.nc
...
function dummy = objmap_o2_annual_clim_multi(klevel, fn, outdir, ...
...
% load data
mn = ncread(fn, 'o2', [1 1 klevel], [360 180 1]);
mn(mn<0) = NaN;

k = klevel;
bind = bindK{k}.data;
wn = fullfile(outdir, ['o2map_ann_klev', num2str(k), '.mat']);
o2map = zeros(360, 180);
...
for b = 1:length(bind)
   ...
   if ~isempty(I)
      ...
      % like climatology; unlike objmap_o2anom_annual_multi.m, this
      % cannot be simplified to A = VU/UU
      invUU = inv(UU);
      v = ones(1,N2);
      lmd1 = (1 - VU*invUU*v');
      lmd2 = (v*invUU*v');
